...
const moveToInterdictScreen = () => {
  const sendToModel = async() => {
    ...
    console.log('보내는데이터', JSON.stringify({pill_names:drugNameList, user_id:user_id}))
    ...
    return (
      <View style = {{flex : 1, alignItems : "center", backgroundColor : "#f5f7ff"}}>
        <View style = {{ flex :1, width : width * 0.90}}>
          ...
          <View style = {{paddingVertical : 10, paddingHorizontal : 10, height : "60%", backgroundColor : "white", marginBottom : 20, borderRadius : 10}}>
            <FlatList 
              data = {drugInfo}
