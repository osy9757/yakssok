...
import { View, Text, Alert, StyleSheet, TouchableOpacity, TouchableWithoutFeedback, Keyboard } from "react-native";
...
const IdInput = ({route, navigation}) => {
    ...
    const checkId = async() => {
        ...
        try {
            await fetch('https://reqres.in/api/posts', requestOptions) //django 링크
                .then(response => {
                    response.json()
                ...
                <View style={styles.inputContainer}> 
                    ...
                    <View style={styles.inputWithBtn}>
                        <StyledInput 
                                ...
                                blurOnSubmit={false}
                                returnKeyType='next'/>
                        {/* <TouchableOpacity onPress={checkId} style={{flex:2, alignItems:'center', justifyContent:'center', backgroundColor:isChk||user_id!==''&&idCondition?'#5471FF':'gray', borderRadius:6, height:50}} disabled={isChk||user_id!==''&&idCondition?false:true}>
                            <Text style={{color:'white'}}>중복 확인</Text>
                        </TouchableOpacity> {/**id체크 했을 때, id값 없을 때, 유효성검사 통과 못할 때 비활성화 */}
